...
class DataRepository:
    def __init__(
        self, events_path: Path, products_path: Path, cold_start_window_days: int = 30
    ):
        ...
        self.trending_scores = self._build_trending_scores(
            events, cold_start_window_days
        )
    ...
    def _build_user_affinity(
        ...
        category_counts = (
            merged.groupby(["user_id", "category"]).size().reset_index(name="count")
        )
        top_category = category_counts.sort_values(
            "count", ascending=False
        ).drop_duplicates("user_id")
        return dict(zip(top_category["user_id"], top_category["category"]))

    @staticmethod
    def _build_trending_scores(
        events: pd.DataFrame, window_days: int
    ) -> dict[str, float]:
